...
namespace CryptoSnas.Webpages
{
    ...
    {
        public const string DBName = "Users.mdf"; //Name of MSSQL Database
        public const string tblName = "Users"; //Name of users table in Database
        public const string conString =
            @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\"
         + DBName + ";Integrated Security=True"; //DB in the App_Data = |DataDirectory|
        ...
        // The Method recieve a user objects. Find the user in the DataBase acording to his userId and update all the other properties in DB.
        {
            ...
            // בניית פקודת SQL
            string SQLStr = "SELECT * FROM " + Helper.tblName + $" Where userid={user.Id}";
            SqlCommand cmd = new SqlCommand(SQLStr, con);
        ...
        public static bool IsUsernameAndPasswordExist(string userName, string password)
        // The Method check if there is a user with userName and Password. 
        // If true the Method return a user with the first Name and Admin property.
        // If not the Method return a user wuth first name "Visitor" and Admin = false
